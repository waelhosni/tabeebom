...
class HorizontalFilterHospital extends Component {
            ...
            <div className="filters-wrapper">
                 {(!isMobile) ?
                    <div className="d-flex align-items-center">
                        <p class="mb-0 filter-title filter-by" style={{paddingLeft:10,paddingRight:10}}><i className="fa fa-filter"></i> {t("Filter")}</p>
                        
                        

                        <div className="horizontal-filter-dropdown  horizontal-filter-dropdown1" ref={this.setColorRef}>
                            {(colorfilter) ?
                                <Button caret className="btn-white dropdown-toggle" onClick={this.colordrop} style={{textAlign:'initial'}}>
                                    <span className="mb-0"><FaHeartbeat style={{fontSize:22,color:'#002D4B',marginLeft:8,marginRight:5}}/> {t("Specialty")} </span>
                                </Button>
                                :
                                <p><Button caret className="btn-white bg-highlight" onClick={this.colordrop}>{t("Specialty")}: <b>{(removecolorlist.length === 1) ? this.Capitalize(removecolorlist[0]) : this.Capitalize(removecolorlist[0]) + '+' + (totalremovecolor - 1)}</b></Button><a  className="filter-close" onClick={() => this.clearcolor()} to=""></a></p>
                            }
                            {(colordrop) ?
                        ...
                        <div className="horizontal-filter-dropdown  horizontal-filter-dropdown1" ref={this.setServiceRef}>
                            {(servicefilter) ?
                                <Button caret className="btn-white dropdown-toggle" onClick={this.servicedrop} style={{textAlign:'initial'}}>
                                    <span className="mb-0"><FaFileMedicalAlt style={{fontSize:22,color:'#002D4B',marginLeft:8,marginRight:5}}/>  {t("ServicesTreatments")}</span>
                                </Button>
                                :
                                <p><Button caret className="btn-white bg-highlight" onClick={this.servicedrop} style={{textAlign:'initial'}}>{t("ServicesTreatments")}: <b>{(removeservicelist.length === 1) ? this.Capitalize(removeservicelist[0]) : this.Capitalize(removeservicelist[0]) + '+' + (totalremoveservice - 1)}</b></Button><a  className="filter-close" onClick={() => this.clearservice()} ></a></p>
                            }
                            {(servicedrop) ?
                                <div className="widget widget_layered_nav widget-layered-nav pgs_widget-layered-nav zoomIn animated">
                                    <div className="widget-title-header" style={{textAlign:'initial'}}>
                                        ...
                                        {this.props.services.map((service, index) => {
                                            return (
                                                ...
                                                <div class="form-check pgs-filter-checkbox">
                                                    <input type="checkbox" onClick={(e) => this.onClickServiceFilter(e, serviceFilterValues)} value={service} defaultChecked={serviceFilterValues.includes(service) ? true : false} class="form-check-input" id={service} />
                                                    <label class="form-check-label" htmlFor={service}>{service}</label>
                                                </div>
                                            )
                                        })}
                                    </div>
                        ...
                        <div className="horizontal-filter-dropdown  horizontal-filter-dropdown1" ref={this.setCategoryRef}>
                            {(categoryfilter) ?
                                <Button caret className="btn-white dropdown-toggle" onClick={this.categorydrop} style={{textAlign:'initial'}}>
                                    <span className="mb-0"><MdLocationOn style={{fontSize:22,color:'#002D4B',marginLeft:8,marginRight:5}}/> {t("Walayat")}</span>
                                </Button>
                                :
                                <p><Button caret className="btn-white bg-highlight" onClick={this.categorydrop} style={{textAlign:'initial'}}>{t("Walayat")}: <b>{(removecategorylist.length === 1) ? this.Capitalize(removecategorylist[0]) : this.Capitalize(removecategorylist[0]) + '+' + (totalremovecategory - 1)}</b></Button><a  className="filter-close" onClick={() => this.clearcategory()} to=""></a></p>
                            }
                            {(categorydrop) ?
                                <div className="widget widget_layered_nav widget-layered-nav pgs_widget-layered-nav zoomIn animated">
                                    <div className="widget-title-header" style={{textAlign:'initial'}}>
                        ...
                        <div className="horizontal-filter-dropdown  horizontal-filter-dropdown1" ref={this.setSizeRef}>
                            {(sizefilter) ?
                                <Button caret className="btn-white dropdown-toggle" onClick={this.sizedrop} style={{textAlign:'initial'}}>
                                    <span className="mb-0"><FaMedkit style={{fontSize:22,color:'#002D4B',marginLeft:8,marginRight:5}}/>  {t("HealthInsurance")}</span>
                                </Button>
                                :
                                <p><Button caret className="btn-white bg-highlight" onClick={this.sizedrop} style={{textAlign:'initial'}}>{t("HealthInsurance")}: <b>{(removesizelist.length === 1) ? this.Capitalize(removesizelist[0]) : this.Capitalize(removesizelist[0]) + '+' + (totalremovesize - 1)}</b></Button><a  className="filter-close" onClick={() => this.clearsize()} ></a></p>
                            }
                            {(sizedrop) ?
                                <div className="widget widget_layered_nav widget-layered-nav pgs_widget-layered-nav zoomIn animated">
                                    <div className="widget-title-header" style={{textAlign:'initial'}}>
                        ...
                        <div className="horizontal-filter-dropdown  horizontal-filter-dropdown1" >
                           ...
                           <div className="btn-white dropdown-toggle  btn-secondary"  style={{textAlign:'initial'}}>
                               <input type="text" id="btn-search" ref={(input) => { this.nameInput = input; }} 
                               className="" value={this.state.SearchValue}  style={{border:0,color: '#009bde',fontWeight: 600}}
                        ...
                        <div>
                            <button className="clear-all-btn" onClick={()=>this.clearallfilter()} style={{textAlign:'initial'}} >{t("Clear_All")}</button>
                        </div>
                        : null}
                    </div>
                :
                        ...
                        <div className={"off-canvas-filter horizontal-mobile-filter"+ (sidedrop ? " filter-open" : " ")}   id="off-canvas-filter">
                           ...
                           <div className="horizontal-filter-dropdown" >
                            ...
                            <div className="horizontal-filter-dropdown" >
                                <div className="widget widget_layered_nav widget-layered-nav pgs_widget-layered-nav">
                                    <div className="widget-title-header" style={{textAlign:'initial'}}>
                                        <h4 className="widget-title">{t("FilterbyRegion")}</h4>
                                    </div>
                                <div className="pgs-widget-layered-nav-list-container has-scrollbar" style={{ height: '210px' ,textAlign:'initial'}}>
                                        {this.props.categorys.map((category, index) => {
                                            return (
                            ...
                            <div className="widget widget_layered_nav widget-layered-nav pgs_widget-layered-nav">
                                <div className="widget-title-header">
                                    <h4 className="widget-title" style={{textAlign:'initial'}}>{t("FilterbyInsurance")}</h4>
                                </div>
                                <div className="pgs-widget-layered-nav-list-container has-scrollbar" style={{ height: '210px',textAlign:'initial' }}>
                                    {this.props.sizes.map((size, index) => {
                                        return (
...
const mapDispatchToProps = state => ({
    ...
    colors: uniqueSpecialty(state.hospitals.hospitals),//Specialty
    categorys: uniqueRegion(state.hospitals.hospitals), //Region
    sizes: uniqueInsurance(state.hospitals.hospitals),//Insurance
    services:uniqueService(state.hospitals.hospitals),//services
...
)(withTranslation("translations")((HorizontalFilterHospital)));