...
import { FaHeartbeat,FaMedkit,FaFileMedicalAlt,FaSearch } from 'react-icons/fa';
import { MdLocationOn } from 'react-icons/md';
import { Link } from "gatsby"
import { withTranslation, Trans } from "react-i18next";

var removecate = [], removecategorylist = [], removesizelist = [], removeservicelist = [];
class HorizontalFilterHomeDoctors extends Component {
    ...
    componentDidMount() {
        ...
        console.log('search:'+this.props.filters.search)
        this.setState({
            SearchValue: this.props.filters.search
        })
         this.props.searchValue(this.props.filters.search);
        //this.nameInput.focus();

        this.onClickColorFilter(null,this.props.filters.color);
        this.onClickCategoryFilter(null,this.props.filters.category)
        this.onClickSizeFilter(null,this.props.filters.size)
        this.onClickServiceFilter(null,this.props.filters.service)
            ...
            <div className="filters-wrapper">
                 {(!isMobile) ?
                    <div className="d-flex align-items-center">
                        {console.log(removecolorlist)}
                        <div className="horizontal-filter-dropdown" ref={this.setColorRef}>
                            {   
                                (colorfilter)?
                                    <Button caret className="btn-white dropdown-toggle secondary1 btn-secondary1"  onClick={this.colordrop} style={{textAlign:'initial'}}>
                                        <span className="mb-0"><FaHeartbeat style={{fontSize:22,color:'#002D4B',marginLeft:8,marginRight:5}}/> {t("Specialty")} </span>
                                    </Button>
                                :
                                    <p><Button caret className="btn-white bg-highlight" onClick={this.colordrop}> <b>
                                        {(removecolorlist.length === 1) ? this.Capitalize(removecolorlist[0]) : this.Capitalize(removecolorlist[0]) + '+' + (totalremovecolor - 1)}</b></Button><a  className="filter-close" onClick={() => this.clearcolor()} to=""></a></p>
                            }
                            {(colordrop) ?
                        ...
                        <div className="horizontal-filter-dropdown" ref={this.setServiceRef}>
                            {(servicefilter) ?
                                <Button caret className="btn-white dropdown-toggle  btn-secondary1"  onClick={this.servicedrop} style={{textAlign:'initial'}}>
                                    <span className="mb-0"><FaFileMedicalAlt style={{fontSize:22,color:'#002D4B',marginLeft:8,marginRight:5}}/>  {t("ServicesTreatments")}</span>
                                </Button>
                                :
                                <p><Button caret className="btn-white bg-highlight" onClick={this.servicedrop} style={{textAlign:'initial'}}> <b>{(removeservicelist.length === 1) ? this.Capitalize(removeservicelist[0]) : this.Capitalize(removeservicelist[0]) + '+' + (totalremoveservice - 1)}</b></Button><a  className="filter-close" onClick={() => this.clearservice()} ></a></p>
                            }
                            {(servicedrop) ?
                                <div className="widget widget_layered_nav widget-layered-nav pgs_widget-layered-nav zoomIn animated">
                                    <div className="widget-title-header" style={{textAlign:'initial'}}>
                        ...
                        <div className="horizontal-filter-dropdown" ref={this.setCategoryRef}>
                            {(categoryfilter) ?
                                <Button caret className="btn-white dropdown-toggle  btn-secondary1" onClick={this.categorydrop} style={{textAlign:'initial'}}>
                                    <span className="mb-0"><MdLocationOn style={{fontSize:22,color:'#002D4B',marginLeft:8,marginRight:5}}/> {t("Walayat")}</span>
                                </Button>
                                :
                                <p><Button caret className="btn-white bg-highlight" onClick={this.categorydrop} style={{textAlign:'initial'}}><b>{(removecategorylist.length === 1) ? this.Capitalize(removecategorylist[0]) : this.Capitalize(removecategorylist[0]) + '+' + (totalremovecategory - 1)}</b></Button><a  className="filter-close" onClick={() => this.clearcategory()} to=""></a></p>
                            }
                            {(categorydrop) ?
                                <div className="widget widget_layered_nav widget-layered-nav pgs_widget-layered-nav zoomIn animated">
                                    <div className="widget-title-header" style={{textAlign:'initial'}}>
                        ...
                        <div className="horizontal-filter-dropdown" ref={this.setSizeRef}>
                            {(sizefilter) ?
                                <Button caret className="btn-white dropdown-toggle  btn-secondary1" onClick={this.sizedrop}style={{textAlign:'initial'}}>
                                    <span className="mb-0"><FaMedkit style={{fontSize:22,color:'#002D4B',marginLeft:8,marginRight:5}}/>  {t("HealthInsurance")}</span>
                                </Button>
                                :
                                <p><Button caret className="btn-white bg-highlight" onClick={this.sizedrop} style={{textAlign:'initial'}}> <b>{(removesizelist.length === 1) ? this.Capitalize(removesizelist[0]) : this.Capitalize(removesizelist[0]) + '+' + (totalremovesize - 1)}</b></Button><a  className="filter-close" onClick={() => this.clearsize()} ></a></p>
                            }
                            {(sizedrop) ?
                                <div className="widget widget_layered_nav widget-layered-nav pgs_widget-layered-nav zoomIn animated">
                                    <div className="widget-title-header" style={{textAlign:'initial'}}>
                                        <h4 className="widget-title"><a  className={closeStyle} onClick={() => this.closesize()}></a></h4>
                                    </div>
                                    <div className="pgs-widget-layered-nav-list-container has-scrollbar" style={{ height: '210px' ,textAlign:'initial'}}>
                        ...
                        <div className="horizontal-filter-dropdown" >
                           ...
                           <div className="btn-white dropdown-toggle  btn-secondary  btn-secondary1" >
                        ...
                        <div className="clear-all-btn1" >
                             <Link  style={{color:'#fff',fontSize:22}} to={`/Doctors`}>
                                 <FaSearch/>
                             </Link>
...
)(withTranslation("translations")(HorizontalFilterHomeDoctors));